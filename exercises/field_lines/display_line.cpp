...
int main(){
    ...
    // Generate seeds
    std::vector<Particle> seeds = getUniformSeed(&data, 10);

    // Calculate field lines
    //std::vector<Line> field_lines = rk4Integrator(&seeds, &data, 0.1f, 100);
    std::vector<Line> field_lines = getEvenSeed(&data, 10.0f, 0.2f, 700);
    ...
    sf::RenderWindow window(sf::VideoMode({window_width, window_height}), "Field Lines Visualization");
