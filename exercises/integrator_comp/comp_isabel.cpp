...
int main(){
    ...
    const std::vector<float> step_sizes = {2.0f, 5.0f, 10.0f, 20.0f};
    const float total_length = 200.0f;
    ...
    for (float step_size : step_sizes)
    {
        ...
        for (const auto& integrator : integrators)
        {
            ...
            const std::string filename = "isabel_" + std::string(integrator.name) + "_step" + formatStepSize(step_size) + ".png";
